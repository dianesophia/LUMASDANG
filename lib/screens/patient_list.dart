    ...
    try {
      ...
      setState(() {
        _patients = snapshot.docs.map((doc) {
          ...
          return Patient(
            firstName: demographic['name'] ?? '',
            lastName: '', // You can split the name if needed
            age: int.tryParse(demographic['age'] ?? '0') ?? 0,
            assessmentRemarks: '', // Can calculate from anthropometric if needed
            ...
            child: Row(
              children: [
                ...
                Container(
                  ...
                  child: ClipOval(
                    child: Image.network(
                      'https://ui-avatars.com/api/?name=${patient.firstName}&background=8BC88A&color=fff&size=56',
                      fit: BoxFit.cover,
                      errorBuilder: (context, error, stackTrace) {
                        return Icon(
                          Icons.person,
